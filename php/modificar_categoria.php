			...
			<?php 
				...
				if(validar_privilegios()){
					if(!isset($_POST['modificar'])){
						...
						$select = update_categoria($con,$_GET['id_modificar']);
						while($muestra = mysqli_fetch_array($select)){
							echo "<form name='formulario_modificar' action='modificar_categoria.php' method='post'>
								<input type='hidden' name='id_categoria' value='".$_GET['id_modificar']."'><br>
						...
						$query = "update categorias set descripcion='".$_POST['descripcion']."' where id_categoria='".$_POST['id_categoria']."'";
						if(!$resultado=mysqli_query($con, $query)) {
							echo "Error".mysqli_error($con);
						} 
			
						else{
							header('Location: /mensajes/php/ver_categorias.php');
							echo "<a href='ver_categorias.php'>Regresar</a>";
						}	
					}
